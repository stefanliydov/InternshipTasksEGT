...
public class PutTests {
    ...
    @Test(dataProvider = "getClassStrategy")
    public void putShouldAddNullKeyCorrectly(Map<Character, Integer> map) {
	for (int i = ONE; i < this.size; i++) {
	    map.put((char) i, i);
	}
	final Integer num = NumberGenerator.generate(2000);
	map.put(null, num);
	final boolean result = map.containsKey(null);
	final Integer getResult = map.get(null);
	Assert.assertTrue(result);
	Assert.assertEquals(getResult, num);

    }

    @Test(dataProvider = "getClassStrategy")
    public void putOnAExistingNullKeyShouldOverrideValue(Map<Character, Integer> map) {
	for (int i = ONE; i < this.size; i++) {
	    map.put((char) i, i);
	}
	final Integer num = NumberGenerator.generate(2000);
	map.put(null, num);
	final Integer num2 = NumberGenerator.generate(2000, 5000);
	map.put(null, num2);
	final boolean result = map.containsKey(null);
	final Integer getResult = map.get(null);
	Assert.assertTrue(result);
	Assert.assertEquals(getResult, num2);
