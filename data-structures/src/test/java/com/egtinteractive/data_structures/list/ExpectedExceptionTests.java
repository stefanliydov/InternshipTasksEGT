...
public class ExpectedExceptionTests {
    ...
    @Test(expectedExceptions = IllegalStateException.class, dataProvider = "getClass")
    public void removeWithoutNextFirstShouldThrowException(List<Integer> list) {
	for (int i = ZERO; i < this.size; i++) {
	    final int num = NumberGenerator.generate(20000);
	    list.add(num);
	}
	final int fullListSize = list.size();
